...
(* Type that represent an aircraft. *)
type t = {
...
let time_start t =
  let (tStart, _) = t.route.(0) in
  Printf.printf "Starting time ===> %f \n" tStart;
  tStart
;;

let copyacft a =
  let (man1,man2,man3)=a.man in
  {speed=a.speed; route=copyroute a.route; predict=copyroute a.predict;
   dev=copyroute a.dev; man=(man1,man2,man3); traj=copytraj a.traj;
   angle=a.angle}
...
let detect_traj_all err acft =
  ...
  Array.iteri (fun i traj ->
      if Array.length traj = 0 then
        trajs.(i) <- Array.make cfg.TP.nb_steps ([], 0., 0.)) trajs;
  let mmtab = TP.minmax cfg trajs in
...
(* Return a boolean that show if aircrafts are in a conflict situation. *)
let is_conflict err acft =
  Array.iter (fun ai -> set_man_par ai err) acft;
  Array.iter (fun ai -> set_traj_par ai err) acft;
  let lines = detect_traj_all err acft in
  (Array.map (fun _ -> []) acft) <> lines
;;
...
let random size n =
  let scramble alpha = alpha -. 0.5 +. Random.float 1. in (* bruit*)
...
(*  Génère les trajectoires possiblent du 2ème avion le 1er étant fixe *)
let enum_rnd_2a size =
  let n = 2 in
  let scramble alpha = alpha -. 0.5 +. Random.float 1. in (* bruit*)
  let rnd_acft1 =
    let xy1 = roundxy (Xy.mul (-.size) (Xy.polar (-. Xy.pi /. 2.))) in
    let xy2 = roundxy (Xy.mul size (Xy.polar (-. Xy.pi /. 2.))) in
    let dist = Xy.norm (Xy.sub xy2 xy1) in
    let speed = max_speed *. (0.7 +. Random.float 0.3 -. dspeed) /. hour in
    let t = Random.float max_start_t in
    let route = [|(t, xy1); (t +. dist /. speed, xy2)|] in
  ...
  let slices = max n (truncate (Xy.pi *. size /. sep /. 1.5)) in
  let s = float slices in
  let alpha = Array.init slices (fun i -> float i *. Xy.pi /. s -. (Xy.pi /. 2.)) in
  Array.sort (fun _ _ -> Random.int 3 - 1) alpha;
  let acft = Array.init n (fun i -> rnd_acft alpha.(i) i) in
  let i = 1 in
  while first_conf_t acft i < fst acft.(i).route.(0) +. min_conf_t do
...
(*  Génère le (ième,jième,kième) example suivant la découpe des intervalles
    - 1 : sur le cap
    - 2 : sur le temps de départ du premier avion
    - 3 : sur le temps de départ du deuxieme avion
    - 4 : sur la vitesse du premier avion
    - 5 : sur la vitesse du second avion
    Paramètres :
    - i1 : indice de la tranche dans l'inter1 [|1; 9|]
    - i2 : indice de la tranche dans l'inter2
    - i3 : indice de la tranche dans l'inter3
*)
let enum_all_2a size i1 i2 i3 =
  let n1 = 10 and n2 = 10 in (* nb de tranches dans chq intervalle *)
  let n = 2 in
  let scramble alpha = alpha -. 0.5 +. Random.float 1. in (* bruit*)
  let rnd_acft1 =
    let xy1 = roundxy (Xy.mul (-.size) (Xy.polar (-. Xy.pi /. 2.))) in
    ...
    let dist = Xy.norm (Xy.sub xy2 xy1) in
    let speed = max_speed *. (0.7 +. Random.float 0.3 -. dspeed) /. hour in
    let t = float i2 *. max_start_t /. float n2 in
    let route = [|(t, xy1); (t +. dist /. speed, xy2)|] in
    let a = new_acft speed in
    a.route <- route;
    set_time a 0.;
  ...
  let rnd_acft alpha i =
    ...
    let dist = Xy.norm (Xy.sub xy2 xy1) in
    let speed = max_speed *. (0.7 +. Random.float 0.3 -. dspeed) /. hour in
    let t = float i3 *. max_start_t /. float n2 in
    let route = [|(t, xy1); (t +. dist /. speed, xy2)|] in
    let a = new_acft speed in
    a.route <- route;
    a.angle <- alpha +. (Xy.pi /. 2.);
    set_time a 0.;
    a in
  let alpha = float i1 *. (Xy.pi /. float n1) -. (Xy.pi /. 2.) in
  let acft = Array.init n (fun i -> rnd_acft alpha i) in
  acft.(0) <- rnd_acft1;
  let i = 1 and j = ref 0 in
  while !j < 100 && first_conf_t acft i < fst acft.(i).route.(0) +. min_conf_t do
    acft.(i) <- (rnd_acft alpha i);
    j := !j + 1;
  done;
  acft

(*  Génère un exemple suivant la découpe des intervalles
    - 0 : sur le cap [|1; 9|]
    - 1 : sur le temps de départ du premier avion
    - 2 : sur le temps de départ du deuxieme avion
    - 3 : sur la vitesse du premier avion [|0; 3|]
    - 4 : sur la vitesse du second avion  [|0; 3|]*)
let gen_situation_2a size index =
  let angleSize = 10 in (*nb de tranches dans l'inter. sur l'angle*)
  let timeStartSize = 10 in (*nb de tranches dans l'inter. sur le tps de départ*)
  let sizeSpeed = 3 in (*nb de tranches dans l'inter sur la vitesse*)
  (* Génération du 1er avion.*)
  let avion1 =
    let xy1 = roundxy (Xy.mul (-. size) (Xy.polar (-. Xy.pi /. 2.))) in
    let xy2 = roundxy (Xy.mul size (Xy.polar (-. Xy.pi /. 2.))) in
    let dist = Xy.norm (Xy.sub xy2 xy1) in
    let speed = max_speed *. (0.7 +. ((float index.(3))*.0.3/. float sizeSpeed)
                              -. dspeed) /. hour in
    let t = float index.(1) *. max_start_t /. float timeStartSize in
    let route = [|(t, xy1); (t +. dist /. speed, xy2)|] in
    let a = new_acft speed in
    a.route <- route;
    set_time a 0.;
    a in
  (* Génération du 2nd avion.*)
  let avion2 =
    let alpha = (Xy.pi/.6.) +. (float index.(0)) *. (Xy.pi /. 15.) -. (Xy.pi /. 2.) in
    let xy1 = roundxy (Xy.mul (-.size) (Xy.polar alpha)) in
    let xy2 = roundxy (Xy.mul size (Xy.polar alpha)) in
    let dist = Xy.norm (Xy.sub xy2 xy1) in
    let speed = max_speed *. (0.7 +. ((float index.(4))*.0.3/. float sizeSpeed)
                              -. dspeed) /. hour in
    let t = float index.(2) *. max_start_t /. float timeStartSize in
    let route = [|(t, xy1); (t +. dist /. speed, xy2)|] in
    let a = new_acft speed in
    a.route <- route;
    a.angle <- alpha +. (Xy.pi /. 2.);
...
let init_index_situation () = [|3; 1; 2; 0; 1|]

let next_situation index =
  let index_max = [|10; 5; 5; 3; 3|] in
  let init_index = [|0; 0; 0; 0; 0|] in
  let i = ref 0 in
  while (!i < Array.length index) do
    if (index.(!i) = index_max.(!i)) then (
