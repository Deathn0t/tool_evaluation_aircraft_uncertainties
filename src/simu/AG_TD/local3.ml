 ...
 *)
  let (mat,confnico)=Acft.detect_traj_glob pb.incert data.acft in
...
let cross a b = 
  ...
  let newbd = Array.mapi (fun i _ -> 
    { t0 = rnd_bary a.d.(i).t0 b.d.(i).t0;
      ...
      a.d in
  scale newbd;
  let lacftb=Array.map (fun a -> Acft.copyacft a) pb.acft in
  ...
  ({d=newad;f = Array.map (fun _ -> 0.) newad; acft=lacfta},
   {d=newbd;f = Array.map (fun _ -> 0.) newbd; acft=lacftb})
...
let print_best data =
  ...
  Printf.printf "dev: %.0f, delay: %.0f\n" (get_nb_dev data) (get_delay pb.acft);
