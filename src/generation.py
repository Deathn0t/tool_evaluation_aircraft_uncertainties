...
def InceptionFactoryA(data, num_1x1, num_3x3red, num_3x3, num_d3x3red, num_d3x3, pool, proj, name):
    ...
    # pool + proj
    pooling = mx.symbol.Pooling(data=data, kernel=(3, 3), stride=(1, 1), pad=(1, 1),
...
#Définition du CNN pour des images de taille (612, 452)

def genNetwork(data, name=''):
    """
    Définition d'un CNN (Convolution Neural Network)
    [CONV_CR, POOL, CONV_CR, POOL, CONV_CR, POOL, FC]
    data: entrée du CNN
    """
    #Couches
    out = get_symbol(3, "4,224,224")

    shape = {"data" : (895, 4, 224, 224)} # TODO : faire attention aux dimensions ?!
    pl = mx.viz.plot_network(out, shape=shape)
    pl.render()
    return out

if __name__ == '__main__':
    #Visualisation du CNN
    #10 images de taille (612,452)
    net = genNetwork(mx.sym.Variable('data'), '1st')
