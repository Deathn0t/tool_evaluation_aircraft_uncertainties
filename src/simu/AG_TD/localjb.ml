...
let pb =
  ...
  let acft = Acft.load Sys.argv.(1) in
  ...
  Array.iteri (fun i a ->
	...
	Acft.apply a)))
    acft;
  {tmin=t +. lag; tmax=tmax; t0min=t0min; pref=pref; acft=acft}
...
let scale man_array =
  Array.iteri (fun i man ->
    ...
    man_array.(i) <-
      ...
      else if t1 < t0 +. lagt0t1 then {t0=t0; t1=t0; h=0.}
      else {t0=t0; t1=t1; h=h})
    man_array;
  {d = man_array; f = Array.map (fun _ -> 0.) man_array}
...
let evalData data =
  ...
  Array.iter Acft.set_man pb.acft;
  ...
  *)
  Array.iteri (fun i _ ->
	...
	data.f.(i) <- data.f.(i) +. 1.;
	conf := !conf +. 1.))
      (Acft.detect_traj pb.acft i)) pb.acft;
  let n = float (Array.length pb.acft) in
  if !conf = 0. then (
